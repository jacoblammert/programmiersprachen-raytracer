 ...
 */
bool sphere::getIntersectVec(ray ray, glm::vec3 &HitPoint, glm::vec3 &HitNormal, float &distance) const {

    glm::vec3 raydirection = ray.direction;
    glm::vec3 rayposition = ray.position;


    float t = glm::dot((pos - rayposition), (raydirection)); // TODO check if right function was imported for dot
 ...
 */
glm::vec3 sphere::getNormal(glm::vec3 position) const {
 ...
 * @param position
 */
void sphere::translate(glm::vec3 position) {
    pos += position;
}
